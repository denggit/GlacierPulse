    ...
    return compute_a3_structural_proxy_metrics(
        {
            ...
            "a3_future_breakout_entry_ts": 1000.0,
            "zone_lower": 2998.0,
            "zone_upper": 3002.0,
            "zone_width": 4.0,
            **row,
...
def test_buy_structural_stop_uses_iceberg_sweep_low_with_buffer():
    out = _structural({"direction": "BUY", "a3_future_breakout_entry_price": 3001.0, "first_iceberg_pie_min_trade_price": 2996.0})
...
def test_sell_structural_stop_uses_iceberg_sweep_high_with_buffer():
    out = _structural({"direction": "SELL", "a3_future_breakout_entry_price": 2999.0, "first_iceberg_pie_max_trade_price": 3004.0})
...
def test_structural_fee_share_uses_roundtrip_fee_over_structural_risk():
    out = _structural({"direction": "BUY", "a3_future_breakout_entry_price": 3000.0, "first_iceberg_pie_min_trade_price": 2994.5})
...
def test_buy_structural_first_hit_target_first():
    ...
    out = _structural({"direction": "BUY", "a3_future_breakout_entry_price": 3001.0, "first_iceberg_pie_min_trade_price": 2996.0}, bars)

    assert out["a3_structural_realized_outcome_1h_future"] == "TARGET_1R_FIRST"
    assert out["a3_structural_realized_r_proxy_1h_future"] == round(1.0 - out["a3_structural_fee_share_r"], 8)
...
def test_buy_structural_first_hit_stop_first():
    ...
    assert out["a3_structural_realized_outcome_1h_future"] == "STOP_1R_FIRST"
    assert out["a3_structural_realized_r_proxy_1h_future"] == round(-1.0 - out["a3_structural_fee_share_r"], 8)
...
def test_buy_structural_ambiguous_both_hit_is_conservative_stop():
    ...
    assert out["a3_structural_realized_outcome_1h_future"] == "AMBIGUOUS_BOTH_HIT"
    assert out["a3_structural_realized_r_proxy_1h_future"] == round(-1.0 - out["a3_structural_fee_share_r"], 8)
    ...
    row = _classify(
        {
            ...
            "a3_future_breakout_seen_flag": True,
            "a3_structural_realized_r_proxy_1h_future": 0.8,
            "a3_structural_realized_outcome_1h_future": "TARGET_1R_FIRST",
        }
    )

    assert row["a3_future_breakout_after_a2_flag"] is False
    assert row["a3_after_a2_future_structural_realized_r_proxy_1h"] == 0.0
    assert row["a3_after_a2_future_structural_realized_outcome_1h"] == "NO_BREAKOUT"
    ...
    row = _classify(
        {
            ...
            "a3_future_breakout_seen_flag": True,
            "a3_future_realized_r_proxy_1h": 0.2,
            "a3_future_fee_share_r": 0.6,
            "a3_structural_stop_price": 2995.5,
            "a3_structural_risk_u": 5.5,
            "a3_structural_fee_share_r": 0.3,
            "a3_structural_realized_r_proxy_15m_future": 0.4,
            "a3_structural_realized_r_proxy_1h_future": 0.7,
            "a3_structural_realized_outcome_15m_future": "CLOSE_EXIT",
            "a3_structural_realized_outcome_1h_future": "TARGET_1R_FIRST",
            "a3_structural_realized_r_proxy_1h_bucket_future": "STRUCT_REALIZED_0_TO_1R",
        }
    )

    assert row["a3_future_breakout_after_a2_flag"] is True
    assert row["a3_after_a2_future_structural_stop_price"] == 2995.5
    assert row["a3_after_a2_future_structural_risk_u"] == 5.5
    assert row["a3_after_a2_future_structural_fee_share_r"] == 0.3
    assert row["a3_after_a2_future_structural_realized_r_proxy_15m"] == 0.4
    assert row["a3_after_a2_future_structural_realized_r_proxy_1h"] == 0.7
    assert row["a3_after_a2_future_structural_realized_outcome_1h"] == "TARGET_1R_FIRST"
    assert row["a3_after_a2_future_structural_realized_r_proxy_1h_bucket"] == "STRUCT_REALIZED_0_TO_1R"
    ...
    row = _classify(
        {
            ...
            "a3_future_fee_share_r": 0.6,
            "a3_structural_fee_share_r": 0.3,
            "a3_structural_realized_r_proxy_1h_future": 0.5,
        }
    )

    assert row["a3_after_a2_future_structural_improved_flag"] is True
    assert row["a3_after_a2_future_structural_vs_v1_delta_r_1h"] == 0.3
    assert row["a3_after_a2_future_structural_fee_share_delta_r"] == -0.3
    ...
    out = _structural(
        {
            **row,
            ...
            "a3_future_breakout_seen_flag": True,
            "a3_future_breakout_entry_ts": 1000.0,
            "a3_future_breakout_entry_price": 3001.0,
        }
    )
    ...
    out = _structural(
        {
            **row,
            ...
            "a3_future_breakout_seen_flag": True,
            "a3_future_breakout_entry_ts": 1000.0,
            "a3_future_breakout_entry_price": 2999.0,
        }
    )
