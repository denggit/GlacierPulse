...
class IcebergTruthScorer:
    ...
    def score(self, candidate: Mapping[str, Any]) -> dict[str, Any]:
        ...
        components = {
            ...
            "impact_suppression_score": self._impact_suppression(c),
            "hidden_absorption_score": self._hidden_absorption(c),
            "reload_replenish_score": self._reload_replenish(c),
            "sweep_reclaim_score": self._sweep_reclaim(c),
            "cvd_divergence_score": self._cvd_divergence(c),
            "non_acceptance_score": self._non_acceptance(c),
        }
        ...
        return {
            ...
            "truth_label": self.label_for_score(total),
    ...
    def _impact_suppression(self, c: "_Merged") -> float:
        score = 0.0
        ...
        if direction == "BUY":
            post_attack_extension = max(0.0, settle - (c.f("post_5s_min_price") or c.f("post_min_price") or settle))
        else:
            post_attack_extension = max(0.0, (c.f("post_5s_max_price") or c.f("post_max_price") or settle) - settle)
        if post_attack_extension <= width:
            score += 4
        if self._inside_zone(c, settle) or 0 <= c.f("reclaim_time_sec", -1.0) <= 5.0:
            score += 4
    ...
    def _reload_replenish(self, c: "_Merged") -> float:
        score = 0.0
    ...
    def _sweep_reclaim(self, c: "_Merged") -> float:
        score = 0.0
    ...
    def _cvd_divergence(self, c: "_Merged") -> float:
        direction = c.s("direction")
        width = max(0.000001, c.f("local_zone_width") or abs(c.f("zone_upper") - c.f("zone_lower")) or 1.5)
        settle = c.f("settle_price") or c.f("trigger_price")
        score = 0.0
    ...
    def _non_acceptance(self, c: "_Merged") -> float:
        score = 0.0
