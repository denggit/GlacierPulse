...
from .forward_metrics import DEFAULT_WINDOWS_SEC, compute_forward_metric
...
class A1RandomBaselineComparator:
    ...
    def _groups(self, events: List[A1EdgeEvent]) -> List[tuple[str, str, set[str]]]:
        groups: List[tuple[str, str, set[str]]] = [("ALL", "ALL", {e.zone_id for e in events})]
        for dimension in (
            "a1_reaction_type",
            "reaction_event_kind",
            "legacy_phase2_type",
            ...
            for event in events:
                values[self._event_value(event, dimension)].add(event.zone_id)
    ...
    def summarize(
        ...
        for dimension, group, zone_ids in self._groups(event_list):
            a1_15 = [r for r in a1_rows if r.get("zone_id") in zone_ids and int(float(r.get("window_sec", 0))) == 900]
            a1_60 = [r for r in a1_rows if r.get("zone_id") in zone_ids and int(float(r.get("window_sec", 0))) == 3600]
            rnd_15 = [r for r in random_rows if r.get("source_zone_id") in zone_ids and int(float(r.get("window_sec", 0))) == 900]
            rnd_60 = [r for r in random_rows if r.get("source_zone_id") in zone_ids and int(float(r.get("window_sec", 0))) == 3600]
            ...
            sample_count = len({r.get("zone_id") for r in a1_15}) if a1_15 else len(zone_ids)
